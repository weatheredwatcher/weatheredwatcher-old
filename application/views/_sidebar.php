<div id="sidebar">
	
	<div class="socialstatistics-widget" data-id="114276686833669364156"></div>
	<script>(function(d,t){var b=d.createElement(t),c=d.getElementsByTagName(t)[0];b.async=b.src="http://socialstatistics.com/widget/widget.min.js";c.parentNode.insertBefore(b,c);})(document,'script');</script>
	<div>
    
    <table id="emurse_resume_badge_gogomonkey" cellspacing="0" cellpadding="4" border="0" style="border: 2px solid #224f6f ;font-family:arial;font-size:11px; width:100%; margin: 10px 0px;">
    	<tr>
    		<th colspan="3" style="border-bottom: 1px solid #224f6f;font-size:12px;color:#fff;background:#4a7b9d;">
    			My Resume
    		</th>
    	</tr>
    	<tr>
    		<td style="text-align: center;">
    			<a href="http://weatheredwatcher.emurse.com/">Web</a>
    		</td>
    		<td style="text-align: center;">
    			<a href="http://weatheredwatcher.emurse.com/download/doc/">DOC</a>
    		</td>
    		<td style="text-align: center;">
    			<a href="http://weatheredwatcher.emurse.com/download/pdf/">PDF</a>
    		</td>
    	</tr>
    	<tr>
    		<td style="text-align: center;">
    			<a href="http://weatheredwatcher.emurse.com/download/rtf/">RTF</a>
    		</td>
    		<td style="text-align: center;">
    			<a href="http://weatheredwatcher.emurse.com/download/odt/">ODT</a>
    		</td>
    		<td style="text-align: center;">
    			<a href="http://weatheredwatcher.emurse.com/download/txt/">TXT</a>
    		</td>
    	</tr>
    	<tr>
    		<th colspan="3">
    			<a href="http://www.emurse.com">
    				<img src="http://www.emurse.com/images/badge_powered.gif" alt="powered by emurse" style="width:75px;height:19px;border:0;" width="75" height="19" />
    			</a>
    		</th>
    	</tr>
    </table>
	</div>
	<a href='https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Function' title='JavaScript Function .toString'><img src='http://static.jsconf.us/promotejsv.gif' height='280' width='160' alt='JavaScript Function .toString'/></a>
	<div>
	<a href="http://get.fedoraproject.org/"><img src="https://fedoraproject.org/static/images/banners/f17release.png" alt="Fedora"></a>
	</div>
	<div id="twitter"></div>
	<div>
<iframe src="http://githubbadge.appspot.com/badge/weatheredwatcher?s=1" style="border: 0;height: 142px;width: 200px;overflow: hidden;" frameBorder=0></iframe>
	</div>
</div>